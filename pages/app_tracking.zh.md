...
两个额外的每应用选项可以影响版本的处理方式：

- **使用版本代码作为操作系统版本**：启用后，Obtainium 将使用 Android 版本代码（一个数字）而不是版本名称字符串与源进行比较。当源提供版本代码但操作系统报告版本名称时，这可以解决不匹配问题，反之亦然。
- **使用发布日期作为版本**：对于许多源可用，此开关将发布日期视为伪版本，而不是使用源提供的版本字符串。当源未提供有意义的版本信息但发布日期可靠时，此功能非常有用。
...
- **WorkManager**（默认）：使用 Android 的标准后台任务调度器。这很省电，但可能会被某些制造商的电池优化策略延迟。
- **前台服务**（可选）：作为带有通知的持久前台服务运行。在积极限制后台工作的设备上，这更可靠，但代价是持续显示通知。需要 Android 11 或更高版本。

### 更新约束

您可以限制后台检查的时间：

- **仅限 Wi-Fi**：仅当连接到 Wi-Fi 网络时检查更新。
- **仅限充电时**：仅当设备充电时检查更新。

您还可以选择在每次打开应用时（"启动时检查"）和/或每次查看应用详情页面时检查更新。
...
静默安装也可以使用 **Shizuku** 执行，它可以授予提权权限。这可以提高某些设备上的可靠性。启用 Shizuku 后，还有一个"伪装成 Google Play"选项，告诉系统您正在使用 Google Play 商店进行安装，这有助于提高某些应用的兼容性。

在可能的情况下，下载并安装每个可用的更新，然后通知用户更新可用或已在后台安装。

### 并行下载

默认情况下，Obtainium 逐个下载和安装更新。启用并行下载将同时处理多个更新，这可以加快批量更新的速度，但可能会使用更多的网络带宽和系统资源。
